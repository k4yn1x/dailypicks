...
# Gate: the full job runs at 07:00 Africa/Lagos (WAT, UTC+1, no DST) = 06:00 UTC.
...
if [[ "$FORCE" != "true" && "$TRIGGER" == "schedule" ]]; then
  if [[ "$UTC_HOUR" == "06" ]]; then
    if [[ "$HOUR" != "07" ]]; then echo "Not 07:xx Africa/Lagos (local hour $HOUR); skipping."; exit 0; fi
    TRIGGER="scheduled-0700-wat"
  else
