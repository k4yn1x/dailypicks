...
STEP 0 — time gate. Run in Bash: `TZ=America/Chicago date +%H:%M`. If this run was started by the schedule (not a manual fire with the word FORCE in the extra message) and the hour is not 05, reply "Skipped: not the 05:30 America/Chicago slot" and stop. (Two UTC slots exist so that exactly one fires at 05:30 local across daylight-saving changes.)
...
STEP 3 — run the pipeline: `cd /home/claude/dailypicks && python3 -m dailypicks.pipeline --trigger scheduled-0530-chicago > run.json; cat run.json`. If "status" is not "published": do NOT touch the board artifact; go to STEP 6 and report the failure with the error text (the previously published board remains live by design).
