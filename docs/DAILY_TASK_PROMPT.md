...
STEP 0 — time gate. Run in Bash: `TZ=Africa/Lagos date +%H:%M`. If this run was started by the schedule (not a manual fire with the word FORCE in the extra message) and the hour is not 07, reply "Skipped: not the 07:00 WAT slot" and stop. (WAT is UTC+1 with no daylight saving, so the single 06:00 UTC schedule always lands at 07:00 local.)
...
STEP 3 — run the pipeline: `cd /home/claude/dailypicks && python3 -m dailypicks.pipeline --trigger scheduled-0700-wat > run.json; cat run.json`. If "status" is not "published": do NOT touch the board artifact; go to STEP 6 and report the failure with the error text (the previously published board remains live by design).
